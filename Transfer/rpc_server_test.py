...
def start_server(config):
    ...
    def dispatch(port_rpc):
        with SimpleThreadedXMLRPCServer(('localhost', port_rpc), requestHandler=RequestHandler) as server:
            ...
            server.register_function(infosDone)
...
def infosDone(state_id, infos):
    ...
    ----------
        state_id: int
            The id of the client (associated with a sequence of actions).
        infos:
            The infos returned by the client.
    ...
    """
    instances[state_id]["infosResults"].put(infos)
...
def start_scene(config, nb_actions):
    ...
    def deferredStart():
        sdict = str(config)
        subprocess.run([config['python_version'], path+"common/rpc_client_test.py", sdict, str(nb_actions), str(port_rpc)], check=True)
...
def add_new_step(history, new_action):
    ...
    if planning:
        ...
        instances[id]["pendingTasks"].put(make_action("fork_and_animate", stateId=nid,
                                            action=new_action))
    else:
        nid = id
...
def get_infos(actions, timeout = None):
    """Returns available results. Blocks until a result is available.
    ...
    id = get_id_from_actions(actions)
    instances[id]["pendingTasks"].put(make_action("get_infos"))
    try:
        infos = instances[id]["infosResults"].get(timeout=timeout)
    except queue.Empty:
        print("TIMEOUT ", timeout)
        infos = {"infos": []}
...
def interact_scene(history, infos):
    """Global function to interact freely with the scene.
    ...
    ----------
        history: list
            The sequence of past actions.
        infos: _
            Global infos we want to impose in the scene.
    """
    global instances
    id = get_id_from_actions(history)
    instances[id]["pendingTasks"].put(make_action("set_infos", infos=infos))
